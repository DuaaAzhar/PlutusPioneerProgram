...
mkValidator dat () ctx = (traceIfFalse "Beneficiary1's signature is missing" signedByBeneficiary2 && traceIfFalse "Deadline has not passed" deadlineReached )
                       ||(traceIfFalse "Beneficiary2's signature is missing" signedByBeneficiary1 && traceIfFalse "Deadline has passed" beforeDeadline )
  where
    info :: TxInfo
